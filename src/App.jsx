...
function ViewProDet({id,empresa,clientes,producciones,contratos,movimientos,crew,eventos,navTo,openM,canDo:_cd,cSave,cDel,saveMov,delMov,setProducciones,setMovimientos}){
  ...
  return <div>
    ...
    {tab===3&&<CrewTab crew={crew||[]} empId={empId} asignados={p.crewIds||[]} onAdd={addCrew} onRem={remCrew} canEdit={_cd&&_cd("producciones")} onHonorario={async m=>{
              const tv=Number(String(m.tarifa||"0").replace(/\D/g,""))||0;
              if(!tv){alert("Este crew no tiene tarifa registrada.");return;}
              if(!confirm(`¿Registrar honorario de ${fmtM(tv)} para ${m.nom}?`)) return;
              const g={id:uid(),empId,eid:id,et:"pro",tipo:"gasto",cat:"Honorarios",desc:"Honorarios "+m.nom,monto:tv,fecha:today()};
              const k=`produ:${empId}:movimientos`;
              const cur=await dbGet(k)||[];
              await dbSet(k,[...cur,g]);
              setMovimientos([...cur,g]);
              alert("✓ Honorario registrado: "+fmtM(tv));
            }}/>}
...
function ViewPgDet({id,empresa,clientes,programas,episodios,auspiciadores,movimientos,crew,eventos,navTo,openM,canDo:_cd,cSave,cDel,saveMov,delMov,setProgramas,setEpisodios,setMovimientos}){
  ...
  return <div>
    ...
    {tab===4&&<CrewTab crew={crew||[]} empId={empId} asignados={pg_.crewIds||[]} onAdd={addCrew} onRem={remCrew} canEdit={_cd&&_cd("programas")} onHonorario={async m=>{
              const tv=Number(String(m.tarifa||"0").replace(/\D/g,""))||0;
              if(!tv){alert("Este crew no tiene tarifa registrada.");return;}
              if(!confirm(`¿Registrar honorario de ${fmtM(tv)} para ${m.nom}?`)) return;
              const g={id:uid(),empId,eid:id,et:"pg",tipo:"gasto",cat:"Honorarios",desc:"Honorarios "+m.nom,monto:tv,fecha:today()};
              const k=`produ:${empId}:movimientos`;
              const cur=await dbGet(k)||[];
              await dbSet(k,[...cur,g]);
              setMovimientos([...cur,g]);
