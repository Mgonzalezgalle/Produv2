  ...
  return (
    <>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 14, marginBottom: 20 }}>
        <Stat label="Oportunidades" value={scopedOpps.length} accent="var(--cy2)" vc="var(--cy)" />
        <Stat label="Pipeline activo" value={scopedOpps.filter(opp => !crmStageMeta(opp.stageId, scopedStages).closedWon && !crmStageMeta(opp.stageId, scopedStages).closedLost).length} accent="var(--cy2)" vc="var(--cy2)" />
      ...
      {!!selectedItems.length && tab === 1 && canManageCrm && (
        <div style={{ display: "flex", gap: 10, marginBottom: 16, flexWrap: "wrap", alignItems: "center", padding: "12px 14px", border: "1px solid var(--bdr2)", borderRadius: 14, background: "linear-gradient(180deg,#ffffff,#f7fbff)", boxShadow: "0 10px 24px rgba(15,23,42,.05)" }}>
      ...
      {tab === 0 ? (isMobile ? (
        <>
          <div style={{ display: "flex", gap: 8, overflowX: "auto", paddingBottom: 8, marginBottom: 14 }}>
            {scopedStages.map(stage => <button key={stage.id} onClick={() => setMobileStageId(stage.id)} style={{ padding: "9px 12px", borderRadius: 999, border: `1px solid ${activeMobileStageId === stage.id ? "rgba(43,109,246,.28)" : "var(--bdr2)"}`, background: activeMobileStageId === stage.id ? "linear-gradient(180deg,var(--cy2),color-mix(in srgb,var(--cy2) 86%, #ffffff 14%))" : "#ffffff", color: activeMobileStageId === stage.id ? "#ffffff" : "var(--gr3)", fontSize: 11, fontWeight: 700, whiteSpace: "nowrap", cursor: "pointer", boxShadow: activeMobileStageId === stage.id ? "0 12px 24px rgba(43,109,246,.18)" : "0 8px 18px rgba(15,23,42,.04)" }}>{stage.name} ({sorted.filter(opp => opp.stageId === stage.id).length})</button>)}
          </div>
          <Card title={mobileStage?.name || "Pipeline"} sub={`${mobileStageItems.length} oportunidad${mobileStageItems.length === 1 ? "" : "es"} · ${fmtM(mobileStageItems.reduce((sum, opp) => sum + Number(opp.monto_estimado || 0), 0))}`} action={canManageCrm ? { label: "+ Nuevo", fn: () => openM("crm-opp", { stageId: activeMobileStageId, status: mobileStage?.closedWon ? "Ganada" : mobileStage?.closedLost ? "Perdida" : "Activa" }) } : null}>
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {mobileStageItems.map(opp => {
                ...
                return <div key={opp.id} onClick={() => setDetailId(opp.id)} style={{ background: "linear-gradient(180deg,#ffffff,#f8fbff)", border: "1px solid var(--bdr2)", borderRadius: 14, padding: 12, cursor: "pointer", display: "flex", flexDirection: "column", gap: 8, boxShadow: "0 12px 24px rgba(15,23,42,.05)" }}>
                  {(() => { const actionTone = nextActionTone(opp); return <>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: "var(--gr3)", gap: 8 }}>
                      ...
                      <span style={{ fontFamily: "var(--fm)", color: "var(--cy2)", flexShrink: 0 }}>{fmtM(opp.monto_estimado || 0)}</span>
                    </div>
            ...
            return <Card key={stage.id} title={collapsed ? <span style={{ writingMode: "vertical-rl", textOrientation: "mixed", display: "inline-block", minHeight: 120 }}>{stage.name}</span> : stage.name} sub={collapsed ? `${stageItems.length}` : `${stageItems.length} oportunidad${stageItems.length === 1 ? "" : "es"} · ${fmtM(totalStage)}`} style={{ padding: 14, minWidth: collapsed ? 86 : 240, width: collapsed ? 86 : 280, flexShrink: 0, transition: "width .2s ease", background: "linear-gradient(180deg,#ffffff,#f8fbff)" }} action={{ label: collapsed ? "›" : "‹", fn: () => toggleStageCollapsed(stage.id) }}>
              <div onDragOver={e => e.preventDefault()} onDrop={async e => { e.preventDefault(); const oppId = e.dataTransfer.getData("text/plain"); const opp = scopedOpps.find(item => item.id === oppId); if (opp && opp.stageId !== stage.id) await updateStage(opp, stage.id); }} style={{ display: "flex", flexDirection: "column", gap: 10, minHeight: 220, alignItems: collapsed ? "stretch" : "initial" }}>
                ...
                {!collapsed && stageItems.map(opp => {
                  ...
                  return <div key={opp.id} draggable onDragStart={e => e.dataTransfer.setData("text/plain", opp.id)} onClick={() => setDetailId(opp.id)} style={{ background: "linear-gradient(180deg,#ffffff,#f8fbff)", border: "1px solid var(--bdr2)", borderRadius: 14, padding: 12, cursor: "pointer", display: "flex", flexDirection: "column", gap: 8, alignItems: "flex-start", boxShadow: "0 12px 24px rgba(15,23,42,.05)" }}>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: "var(--gr3)", width: "100%", gap: 8 }}>
                      ...
                      <span style={{ fontFamily: "var(--fm)", color: "var(--cy2)" }}>{fmtM(opp.monto_estimado || 0)}</span>
                    </div>
              ...
              <tbody>
                {paged.map(opp => {
                  ...
                  return <tr key={opp.id}>
                    ...
                    <TD mono style={{ color: "var(--cy2)", fontSize: 12 }}>{fmtM(opp.monto_estimado || 0)}</TD>
