...
export function MPiezaContenido({ open, data, listas, crewOptions, onClose, onSave, normalizeSocialPiece, uid, today }) {
  ...
  return <Modal open={open} onClose={onClose} title={data?.id ? "Editar Pieza" : "Nueva Pieza"} sub="Pieza dentro de una campaña" wide>
    ...
    <R3><FG label="Mes de la pieza"><FSl value={f.mes || ""} onChange={e => u("mes", e.target.value)}><option value="">Seleccionar...</option>{MESES.map(mes => <option key={mes} value={mes}>{mes}</option>)}</FSl></FG><FG label="Fecha Inicio"><FI type="date" value={f.ini || ""} onChange={e => u("ini", e.target.value)} /></FG><FG label="Fecha Entrega"><FI type="date" value={f.fin || ""} onChange={e => u("fin", e.target.value)} /></FG></R3>
