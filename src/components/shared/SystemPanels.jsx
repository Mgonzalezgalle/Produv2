...
export function AlertasPanel({ alertas, leidas = [], resueltas = [], onMarcar, onMarcarTodas, onResolver, onResolverTodas, onOcultar, onOcultarTodas, onClose, fmtD }) {
  ...
  const resueltasList = alertas.filter(a=>resueltas.includes(a.id));
  const [filtro,setFiltro]=useState("todas");
  const filteredActive=activas.filter(a=>filtro==="todas"||a.tipo===filtro||a.area===filtro);
  const counters={
    urgentes:activas.filter(a=>a.tipo==="urgente").length,
    equipo:activas.filter(a=>a.area==="equipo").length,
    comercial:activas.filter(a=>a.area==="comercial").length,
  };
  return (
      ...
      <div style={{padding:"14px 16px",borderBottom:"1px solid var(--bdr)",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          ...
          <div>
            ...
            <div style={{fontSize:10,color:"var(--gr2)"}}>{activas.length} activa(s) · {resueltasList.length} resuelta(s)</div>
          </div>
        </div>
        ...
        {filtro!=="resueltas"&&filteredActive.map(a=>{
          ...
          return <div key={a.id} style={{display:"flex",gap:10,padding:"12px 16px",borderBottom:"1px solid var(--bdr)",background:bg,alignItems:"flex-start"}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:6,flexShrink:0}}>
              {!leidas.includes(a.id)&&<button onClick={()=>onMarcar(a.id)} title="Marcar como revisada" style={{background:"none",border:"1px solid var(--bdr2)",borderRadius:6,color:"var(--gr2)",cursor:"pointer",fontSize:11,padding:"2px 7px",whiteSpace:"nowrap"}}>Revisada</button>}
              <button onClick={()=>onResolver(a.id)} title="Resolver alerta" style={{background:"var(--cy)",border:"1px solid var(--cy)",borderRadius:6,color:"#fff",cursor:"pointer",fontSize:11,fontWeight:800,padding:"2px 7px",whiteSpace:"nowrap"}}>Resolver</button>
              <button onClick={()=>onOcultar(a.id)} title="Archivar alerta" style={{background:"none",border:"1px solid var(--bdr2)",borderRadius:6,color:"var(--gr2)",cursor:"pointer",fontSize:11,padding:"2px 7px",whiteSpace:"nowrap"}}>Archivar</button>
            </div>
          </div>;
        })}
        {filtro!=="resueltas"&&filteredActive.length===0&&activas.length>0&&<div style={{padding:18,textAlign:"center",color:"var(--gr2)",fontSize:12}}>No hay alertas en este filtro.</div>}
        {filtro==="resueltas"&&resueltasList.map(a=><div key={a.id} style={{display:"flex",gap:10,padding:"10px 16px",borderBottom:"1px solid var(--bdr)",opacity:.72,alignItems:"center"}}>
          <span style={{fontSize:14,flexShrink:0}}>✓</span>
          <div style={{flex:1,minWidth:0}}>
            <div style={{fontSize:12,color:"var(--gr3)"}}>{a.titulo}</div>
            <div style={{fontSize:11,color:"var(--gr2)"}}>{a.sub} · {fmtD(a.fecha)}</div>
          </div>
          <button onClick={()=>onOcultar(a.id)} title="Archivar alerta" style={{background:"none",border:"1px solid var(--bdr2)",borderRadius:6,color:"var(--gr2)",cursor:"pointer",fontSize:11,padding:"2px 7px",flexShrink:0,whiteSpace:"nowrap"}}>Archivar</button>
        </div>)}
