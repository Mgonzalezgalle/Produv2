...
function confirmTreasuryStateChange({ label = "documento", current = "", next = "" } = {}) {
  if (!next || next === current) return false;
  return window.confirm(`Vas a cambiar el estado de ${label} de "${current || "Sin estado"}" a "${next}".\n\nEsta acción afecta la lectura financiera del documento. ¿Confirmas el cambio?`);
}
                      ...
                      <div className="treasury-detail">
                              ...
                              <label style={{ minWidth: 220 }}>
                                ...
                                <select
                                  ...
                                  onChange={e => {
                                    ...
                                    if (!confirmTreasuryStateChange({ label: row.correlativo || "este documento", current: row.cobranza, next: nextState })) return;
                                    onUpdateCobranza(row, nextState);
                                  }}
                                  style={TREASURY_SELECT_STYLE}
                                >
                        ...
                        <DetailTable
                          columns={[
                            ...
                            { key: "amount", label: "Monto", render: item => <span className="treasury-mono treasury-pending-paid">{formatTreasuryMoney(item.amount, item.currency || row.currency)}</span> },
                          ]}
    ...
    <div className="treasury-table-wrap">
      <table className="treasury-table">
        ...
        <tbody>
          {rows.map(row => {
            const open = openId === row.id;
            const pendingMode = row.status === "Pagada" ? "paid" : row.status === "Anulada" ? "idle" : row.status === "Vencida" ? "overdue" : row.pending > 0 ? "pending" : "idle";
...
export function IssuedOrdersTable({ rows = [], onEdit, onDelete, onSupplierEmail, onOpenDetail, onOpenPdf, selectedIds = [], toggleSelected, toggleAll, pageIds = [] }) {
  ...
  return (
    <div className="treasury-table-wrap">
      <table className="treasury-table">
        <thead><tr><th style={{ width: 36 }}><input type="checkbox" checked={pageIds.length > 0 && pageIds.every(id => selectedIds.includes(id))} onChange={e => toggleAll(e.target.checked)} /></th><th>Proveedor</th><th>OC</th><th>Fecha</th><th>Envío</th><th>Monto</th><th></th></tr></thead>
        <tbody>{rows.map(row => <tr key={row.id}><td><input type="checkbox" checked={selectedIds.includes(row.id)} onChange={() => toggleSelected(row.id)} /></td><td><div style={{ fontWeight: 700 }}>{row.supplier}</div><div className="treasury-muted" style={{ fontSize: 11 }}>{row.costCenter || row.category || "Sin centro de costo"}</div></td><td><div style={{ fontWeight: 700 }}>{row.number}</div><div className="treasury-muted" style={{ fontSize: 11 }}>{row.productionName || `${(row.items || []).length} línea${(row.items || []).length === 1 ? "" : "s"}`}</div></td><td>{row.issueDate ? fmtD(row.issueDate) : "—"}</td><td><div style={{ display: "grid", gap: 2 }}>{row.lastSentAt ? <span style={{ fontSize: 12, fontWeight: 700, color: "var(--wh)" }}>Enviada {fmtD(String(row.lastSentAt).slice(0, 10))}</span> : <span style={{ fontSize: 12, color: "var(--gr2)" }}>Sin enviar</span>}{row.lastSentTo ? <span style={{ fontSize: 11, color: "var(--gr2)" }}>{row.lastSentTo}</span> : null}</div></td><td className="treasury-mono">{fmtM(row.amount)}</td><td><div style={{ display: "flex", gap: 6, justifyContent: "flex-end", flexWrap: "wrap" }}>{onOpenDetail ? <GBtn sm onClick={() => onOpenDetail(row)}>Ver detalle</GBtn> : null}{onOpenPdf ? <GBtn sm onClick={() => onOpenPdf(row)}>PDF</GBtn> : null}{onSupplierEmail ? <GBtn sm onClick={() => onSupplierEmail(row)}>Correo</GBtn> : null}<GBtn sm onClick={() => onEdit(row)}>Editar</GBtn><DBtn sm onClick={() => onDelete(row.id)}>Eliminar</DBtn></div></td></tr>)}</tbody>
      </table>
    </div>
  );
}
