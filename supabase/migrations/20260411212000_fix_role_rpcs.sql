...
declare
  ...
  perm text;
  input_role_key text := role_key;
  input_role_label text := role_label;
  input_role_color text := role_color;
  input_role_badge text := role_badge;
begin
  if coalesce(trim(legacy_emp_id), '') = '' then
    raise exception 'legacy_emp_id_required';
  end if;
  if coalesce(trim(input_role_key), '') = '' then
    raise exception 'role_key_required';
  end if;
  if coalesce(trim(input_role_label), '') = '' then
  ...
  insert into public.roles (
    ...
    input_role_key,
    input_role_label,
    coalesce(nullif(input_role_color, ''), '#7c7c8a'),
    coalesce(nullif(input_role_badge, ''), 'gray'),
    false
  )
