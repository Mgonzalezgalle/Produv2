create table if not exists public.identity_membership_blueprints (
  ...
  source_plan_status text not null default 'incomplete',
...
create index if not exists idx_identity_membership_blueprints_tenant_id
  on public.identity_membership_blueprints(tenant_id);

drop trigger if exists identity_membership_blueprints_touch_updated_at on public.identity_membership_blueprints;
create trigger identity_membership_blueprints_touch_updated_at
before update on public.identity_membership_blueprints
for each row execute function public.touch_updated_at();

alter table public.identity_membership_blueprints enable row level security;

create policy "temporary deny identity_membership_blueprints"
on public.identity_membership_blueprints
for all
using (false)
with check (false);

create or replace function public.prepare_identity_membership_blueprints(
  legacy_emp_id text
)
returns jsonb
language plpgsql
...
begin
  ...
  delete from public.identity_membership_blueprints
   where tenant_id = tenant_match.id;

  insert into public.identity_membership_blueprints (
    tenant_id,
    legacy_user_id,
    email,
    full_name,
  ...
  select
    ...
    ipp.full_name,
    ipp.target_role_key,
    case when ipp.plan_status = 'ready' then 'prepared' else 'blocked' end as blueprint_status,
    ipp.plan_status,
    jsonb_build_object(
...
grant execute on function public.prepare_identity_membership_blueprints(text) to anon, authenticated, service_role;
